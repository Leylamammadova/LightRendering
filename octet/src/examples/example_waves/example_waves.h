...
namespace octet {
  ...
  public:
    ...
    void app_init() {
      ...
      // Cube
      vertShader = file_reader.load_file("shaders/reflect.vs").c_str();
      fragShader = file_reader.load_file("shaders/reflect.fs").c_str();

      obj_file_io::opengl_data cube_data = file_reader.parse_file_data(file_reader.load_mesh_file("cube.obj"));

      cube = new entity();
      cube->init(-20, 10, 0, vertShader, fragShader, cube_data.vertex_object, cube_data.indices);
      cube->scale(10, 10, 10);
      cube->rotate(45);
